...
dset_loaders = {
    x : torch.utils.data.DataLoader(dsets[x], batch_size = cf.batch_size, shuffle=(x=='train'), num_workers=1)
    for x in ['train', 'val']
}
...
if (args.testOnly):
    ...
    checkpoint = torch.load('./checkpoint/'+dataset_dir+'/'+file_name+'.pth.tar')
            ...
            if (phase == 'val'):
                ...
                if epoch_acc > best_acc:
                    ...
                    if not os.path.isdir(save_point):
                        os.mkdir(save_point)
                    torch.save(state, save_point+file_name+'.pth.tar')
...
if(args.resetClassifier):
    ...
    if(args.addlayer):
        ...
        if(args.net_type == 'alexnet' or args.net_type == 'vggnet'):
            num_ftrs = model_ft.fc[6].in_features
            feature_model = list(model_ft.fc.children())
            feature_model.pop()
