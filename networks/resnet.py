...
def resnet(pretrained=False, depth=50, **kwargs):
    ...
    if (pretrained):
        ...
        #model = torch.load('places365.pth.tar')
        model = torch.load('places365.pth.tar')#model_zoo.load_url(model_urls['resnet%d' %depth]))
